...
armServer2 = rospy.ServiceProxy('/uav2/mavros/cmd/arming', CommandBool)
setModeServer2 = rospy.ServiceProxy('/uav2/mavros/set_mode', SetMode)
local_target_pub = rospy.Publisher('/uav0/mavros/rc/override', OverrideRCIn, queue_size=100)
def __init__():
	rospy.init_node('PX4_control' ,anonymous=True)
	rospy.Subscriber("/uav0/mavros/state", State, mavros_state_callback)
        if armServer0(True):
		print("Vehicle0 arming succeed!")
        else:
		print("Vehicle0 arming failed!")
        if armServer1(True):
		print("Vehicle1 arming succeed!")
        else:
		print("Vehicle1 arming failed!")		
        if armServer2(True):
		print("Vehicle2 arming succeed!")
        else:
		print("Vehicle2 arming failed!")	
			
	if setModeServer0(custom_mode='OFFBOARD'):
		print("Vehicle0 offboard succeed!")
	else:
		print("Vehicle0 offboard failed!")
	if setModeServer1(custom_mode='OFFBOARD'):
		print("Vehicle1 offboard succeed!")
	else:
		print("Vehicle1 offboard failed!")
	if setModeServer2(custom_mode='OFFBOARD'):
		print("Vehicle2 offboard succeed!")
	else:
		print("Vehicle2 offboard failed!")	
	print("Initialized")
